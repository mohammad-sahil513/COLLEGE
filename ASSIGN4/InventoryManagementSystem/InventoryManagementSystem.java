...
public class InventoryManagementSystem {
	...
	private static List<Purchase> loadPurchaseHistory() {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(PURCHASE_HISTORY_FILE))) {
			...
			while ((line = reader.readLine()) != null) {
				String[] data = line.split(DELIMITER);
				purchaseHistory.add(new Purchase(data[0], Integer.parseInt(data[1]), Double.parseDouble(data[2]),
	...
	private static void savePurchaseHistoryToFile(List<Purchase> purchaseHistory) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(PURCHASE_HISTORY_FILE))) {
			for (Purchase purchase : purchaseHistory) {
				writer.write(purchase.getProductName() + DELIMITER + purchase.getQuantity() + DELIMITER
						+ purchase.getTotalPrice() + DELIMITER + purchase.getPurchaseDate().getTime());
				writer.newLine();
			}
		} catch (IOException e) {
...
class Purchase {
	...
	@Override
	public String toString() {
		return "Product: " + productName + ", Quantity: " + quantity + ", Total Price: $" + totalPrice + ", Purchase Date: "
			+ purchaseDate;
	}
}
