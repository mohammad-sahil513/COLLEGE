...
class marksManagementSystem {
	...
	private static void addStudentMarks(Scanner scanner, List<Mark> marks) {
	...
	private static void viewMarks(Scanner scanner, List<Mark> marks) {
	...
	private static List<Mark> loadMarksFromFile() {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				marks.add(new Mark(Integer.parseInt(data[0]), data[1], Double.parseDouble(data[2]), Double.parseDouble(data[3]),
	...
	private static void saveMarksToFile(List<Mark> marks) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_NAME))) {
			for (Mark mark : marks) {
				writer.write(mark.getRollNo() + DELIMITER + mark.getName() + DELIMITER + mark.getsweMark() + DELIMITER
						+ mark.getsnkMark() + DELIMITER + mark.getevsMark());
				writer.newLine();
			}
		} catch (IOException e) {
...
class Mark {
	...
	public Mark(int rollNo, String name, double swe, double snk, double evs) {
		this.rollNo = rollNo;
		this.name = name;
		this.swe = swe;
		this.snk = snk;
	...
	public String toString() {
		return rollNo + " " + name + " " + swe + " " + snk + " " + evs + " ";
	}
